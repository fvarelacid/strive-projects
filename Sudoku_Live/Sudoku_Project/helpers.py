...
def find_corners_of_contour(contour):
	p = cv2.arcLength(contour, True)
	appr = cv2.approxPolyDP(contour, 0.02 * p, True)

	appr = sorted(appr, key = lambda x: x[0][0])

	pa, pb = sorted(appr[:2], key = lambda x: x[0][1])
	pc, pd = sorted(appr[2:], key = lambda x: x[0][1])

	return np.array([pa[0], pb[0], pc[0], pd[0]], dtype = np.float32)
